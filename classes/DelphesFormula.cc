...
Int_t DelphesFormula::Compile(const char *expression)
{
  ...
  buffer.ReplaceAll("initialRadius", "[5]");
...
Double_t DelphesFormula::Eval(Double_t pt, Double_t eta, Double_t phi, Double_t energy, Candidate *candidate)
{
  ...
  Double_t d0 = 0., dz = 0., ctgTheta = 0., radius = 0., density = 0., initialRadius = 0.;
  if(candidate)
  {
    d0 = candidate->D0;
    dz = candidate->DZ;
    ...
    initialRadius = candidate->InitialPosition.Pt();
  }

  Double_t x[4] = {pt, eta, phi, energy};
  Double_t params[6] = {d0, dz, ctgTheta, radius, density, initialRadius};
